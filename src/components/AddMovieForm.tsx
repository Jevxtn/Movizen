...
interface AddMovieFormProps {
  onAddMovie: (movie: Omit<Movie, 'id'>) => void; // Function to call when a movie is added
}

export const AddMovieForm = ({ onAddMovie }: AddMovieFormProps) => {
      ...
      <div className="grid grid-cols-1 gap-6">
                ...
                onClick={() => {
                  ...
                  if (!currentMode && releaseDate) {
                    ...
                    if (!isNaN(dateObj.getTime())) {
                      ...
                      setReleaseDate(''); // Clear if invalid date
                    }
                  } else if (currentMode && releaseDate) {
                    // When switching from date picker to manual, keep the date as is
                    // (it's already in YYYY-MM-DD format which is supported)
                  } else {
                    setReleaseDate(''); // Clear the input when switching modes with no valid date
                  }
                }}
        ...
        {selectedMovie && (
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            <div className="flex items-center space-x-3">              {selectedMovie.Poster && (
                <Image 
